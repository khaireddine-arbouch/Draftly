...
const ChatWindow = ({ generatedUIId, isOpen, onClose }: Props) => {
  ...
  return (
    <div
      className={cn(
        "fixed right-5 top-1/2 transform -translate-y-1/2 w-96 h-[600px] bg-neutral-900/80 border-white/16 border rounded-lg z-50 transition-all duration-300 flex flex-col",
        isOpen ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"
      )}
    >
      <div className="flex items-center justify-between p-4 border-b border-white/16">
        <div className="flex items-center gap-2">
          <RefreshCw className="w-5 h-5 text-white/88" />
      ...
      <div className="p-4 border-t border-white/16">
