...
export const handlePolarEvent = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Extract subscription and order-like data
      ...
      async () => {
        ...
        if (email) {
          try {
            ...
            console.error(
              "X [Inngest] Failed to resolve user by email:",
              error
            );
        ...
        console.log("X [Inngest] No email found to lookup user");
        return null;
      }
    );
    ...
    if (!polarSubscriptionId) {
      console.log("X [Inngest] No polar subscription ID found, skipping");
      return;
    }
    ...
    const subscriptionId = await step.run("upsert-subscription", async () => {
      try {
        ...
        if (allUserSubs && allUserSubs.length > 1) {
          allUserSubs.forEach(
            (
              sub: { _id: string; polarSubscriptionId: string; status: string },
              index: number
            ) => {
              console.error(
                `${index + 1}. ID: ${sub._id}, Polar ID: ${
    ...
    if (entitled && (looksCreate || looksRenew)) {
      ...
      if (grant.ok && !("skipped" in grant && grant.skipped)) {
        await step.sendEvent("credits-granted", {
          ...
          id: `credits-granted:${polarSubscriptionId}:${
